...
const crmSecondaryItems = [
  ...
  { label: 'Согласования', icon: 'check' as const, path: '/approvals' },
...
const roleLabels: Record<string, string> = { user: 'Без роли', student: 'Студент', teacher: 'Преподаватель', manager: 'КАМ / менеджер', leader: 'Руководитель', admin: 'Администратор' }
...
function breadcrumb(path: string) {
  const map: Record<string, string> = {'/':'Обзор','/profile':'Мой профиль','/universities':'Вузы','/programs':'Программы','/my-program':'Моя программа','/students':'Студенты','/analytics':'Аналитика','/reports':'Отчеты','/integrations':'Интеграции','/approvals':'Согласования','/import':'Импорт','/workflows':'Процессы','/users':'Пользователи','/student-registry':'Студенты','/tasks':'Задачи','/documents':'Документы','/help':'Помощь'}
  return path.startsWith('/universities/') ? 'Карточка вуза' : map[path] ?? 'Раздел'
}

export function AppShell({ children, path, navigate, taskCount, approvalCount = 0, settingsSignal = 0, currentUser, onLogout }: { taskCount: number; approvalCount?: number; children: ReactNode; path: string; navigate: (path: string) => void; settingsSignal?: number; currentUser: AuthUser; onLogout: () => Promise<void> }) {
  ...
  const canUseCrm = isAdmin || isLeader || isManager
  ...
  const visibleSecondaryItems = canUseCrm ? crmSecondaryItems.filter(item => {
    if (item.path === '/users') return canManageUsers
    if (item.path === '/workflows') return canManageContent
    return true
  }) : []
  ...
  const submitSearch = () => {
    ...
    else if (value.includes('помощ') || value.includes('руковод') || value.includes('инструк')) navigate('/help')
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-label">{canUseCrm ? 'РАБОЧЕЕ ПРОСТРАНСТВО' : activeRole === 'student' ? 'ОБУЧЕНИЕ' : activeRole === 'teacher' ? 'ПРЕПОДАВАНИЕ' : 'УЧЁТНАЯ ЗАПИСЬ'}</div>
      ...
      {visibleSecondaryItems.length > 0 && <><div className="sidebar-label second">УПРАВЛЕНИЕ</div><nav className="nav-list" aria-label="Навигация">{visibleSecondaryItems.map((item) => <button key={item.path} className={`nav-item ${isActive(item.path) ? 'active' : ''}`} onClick={() => navigate(item.path)}><Icon name={item.icon} size={19} /><span>{item.label}</span>{item.path === '/tasks' && taskCount > 0 && <span className="nav-badge">{taskCount}</span>}{item.path === '/approvals' && approvalCount > 0 && <span className="nav-badge">{approvalCount}</span>}</button>)}</nav></>}
      <div className="sidebar-bottom">
        <button className={`nav-item ${path === '/help' ? 'active' : ''}`} onClick={() => navigate('/help')}><Icon name="book" size={19} /><span>Помощь</span></button>
    ...
    <main className="main"><header className="topbar"><div className="topbar-leading"><button ref={menuButton} className="mobile-menu-button" aria-expanded={menuOpen} aria-controls="mobile-navigation" aria-label={menuOpen ? 'Закрыть меню' : 'Открыть меню'} onClick={() => setMenuOpen(open => !open)}><span aria-hidden="true">{menuOpen ? '×' : '☰'}</span></button><div className="breadcrumbs"><span>CRM</span><span>/</span><strong>{breadcrumbLabel}</strong></div></div><div className="topbar-actions">{canUseCrm && <form className="search" onSubmit={event => { event.preventDefault(); submitSearch() }}><Icon name="search" size={18} /><input aria-label="Поиск" value={search} onChange={event => setSearch(event.target.value)} placeholder="Поиск..." /></form>}{canUseCrm && <div className="notification-wrap"><button className="icon-button" aria-label="Уведомления" aria-expanded={notificationsOpen} onClick={() => setNotificationsOpen(open => !open)}><Icon name="bell" size={19} />{(taskCount > 0 || approvalCount > 0) && <span className="notification-dot" />}</button>{notificationsOpen && <div className="top-popover"><strong>Уведомления</strong><p>{approvalCount ? `Запросов на согласование: ${approvalCount}.` : taskCount ? `Открытых задач: ${taskCount}. Проверьте сроки и ответственных.` : 'Новых уведомлений нет.'}</p>{approvalCount > 0 ? <button className="popover-action" onClick={() => navigate('/approvals')}>Перейти к согласованиям <Icon name="arrow" size={14} /></button> : <button className="popover-action" onClick={() => navigate('/tasks')}>Перейти к задачам <Icon name="arrow" size={14} /></button>}</div>}</div>}<button className="top-avatar top-avatar-button" aria-label="Открыть мой профиль" onClick={() => navigate('/profile')}>{initials}</button></div></header>{menuOpen && <nav id="mobile-navigation" className="mobile-navigation" aria-label="Разделы CRM" onKeyDown={event => { if (event.key === 'Escape') closeMenu() }}>
      {[...navItems, ...visibleSecondaryItems].map(item => <button key={item.path} aria-current={isActive(item.path) ? 'page' : undefined} className={`nav-item ${isActive(item.path) ? 'active' : ''}`} onClick={() => { closeMenu(); navigate(item.path) }}><Icon name={item.icon} size={20} />{item.label}{item.path === '/tasks' && taskCount > 0 && <span className="nav-badge">{taskCount}</span>}{item.path === '/approvals' && approvalCount > 0 && <span className="nav-badge">{approvalCount}</span>}</button>)}
      {canUseCrm && <button className={`nav-item ${path === '/profile' ? 'active' : ''}`} onClick={() => { closeMenu(); navigate('/profile') }}><span className="mobile-profile-mark">{initials}</span>Мой профиль</button>}
      <button className={`nav-item ${path === '/help' ? 'active' : ''}`} onClick={() => { closeMenu(); navigate('/help') }}><Icon name="book" size={20} />Помощь</button>

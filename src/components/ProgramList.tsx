...
export function ProgramList({ programs, selectedId, onSelect }: {
  ...
  return <>
    <div className="table-wrap program-table"><table><thead><tr><th>ИТ-направление</th><th>Программа</th><th>ИТ-продукт</th><th>Обучающиеся</th><th>Потоки</th><th>Востребованность</th><th>Этап</th></tr></thead>
      <tbody>{programs.map(program => <tr key={program.id} className={selectedId === program.id ? 'selected-program-row' : ''}>
        <td><strong>{program.direction || 'Не указано'}</strong></td><td><button className="program-link" onClick={() => onSelect(program)}>{program.name}</button><span className="table-secondary">{program.applications.toLocaleString('ru-RU')} заявок</span></td>
    ...
    <div className="program-mobile-list">{programs.map(program => <article key={program.id} className={`program-mobile-card ${selectedId === program.id ? 'selected-program-row' : ''}`}>
      <small className="table-secondary">{program.direction || 'Направление не указано'}</small>

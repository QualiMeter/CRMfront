...
export function UsersPage({ users, universities, onCreate, onUpdate }: {
  ...
  async function copyRegistrationLink() {
    ...
    try {
      ...
      setMessage('Ссылка на регистрацию скопирована. Отправьте её приглашённому сотруднику.')
    } catch {
      setError(`Не удалось скопировать ссылку автоматически: ${registrationUrl}`)
    }
  }
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      else { await onCreate(form); setMessage('Приглашение создано. Отправьте сотруднику ссылку на регистрацию.') }
  ...
  return <div className="content users-workspace">
    <div className="page-heading"><div><div className="eyebrow">ДОСТУП И РОЛИ</div><h1>Пользователи</h1><p className="muted">Управление сотрудниками и доступом к данным учебных заведений</p></div><div className="users-heading-actions"><button className="outline-button" onClick={() => void copyRegistrationLink()}>Скопировать ссылку</button><button className="primary-button" onClick={openCreate}><Icon name="plus" size={17} /> Пригласить пользователя</button></div></div>
    ...
    {(creating || editing) && <div className="crm-overlay" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) closeModal() }}><section className="crm-modal user-modal" role="dialog" aria-modal="true" aria-labelledby="user-modal-title"><div className="modal-heading"><div><div className="eyebrow">{editing ? 'РЕДАКТИРОВАНИЕ ДОСТУПА' : 'ПРИГЛАШЕНИЕ СОТРУДНИКА'}</div><h2 id="user-modal-title">{editing ? editing.name : 'Пригласить сотрудника'}</h2></div><button className="modal-close" onClick={closeModal} aria-label="Закрыть">×</button></div><form onSubmit={submit}><fieldset disabled={saving}><label>ФИО<input required maxLength={160} value={form.name} onChange={event => setForm(current => ({ ...current, name: event.target.value }))} /></label><label>Email<input required type="email" disabled={!!editing} value={form.email} onChange={event => setForm(current => ({ ...current, email: event.target.value }))} placeholder="user@company.ru" /></label><label>Роль<select value={form.role} onChange={event => setForm(current => ({ ...current, role: event.target.value as UserRole, universityIds: event.target.value === 'admin' ? [] : current.universityIds }))}>{Object.entries(roleLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>{form.role !== 'admin' && <fieldset className="university-access-field"><legend>Доступ к вузам</legend>{universities.map(university => <label key={university.id}><input type="checkbox" checked={form.universityIds.includes(university.id)} onChange={() => toggleUniversity(university.id)} /><span><strong>{university.shortName}</strong><small>{university.name}</small></span></label>)}</fieldset>}<p className="invite-note">CRM сохранит профиль, роль и область доступа со статусом «Приглашён». Передайте сотруднику ссылку на регистрацию — он должен указать этот же email.</p><div className="modal-actions"><button type="button" className="task-action" onClick={closeModal}>Отмена</button><button type="submit" className="primary-button">{saving ? 'Сохранение…' : editing ? 'Сохранить' : 'Создать приглашение'}</button></div></fieldset>{error && <p className="form-error" role="alert">{error}</p>}</form></section></div>}
  </div>
}

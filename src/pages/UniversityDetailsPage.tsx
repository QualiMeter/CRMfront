...
export function UniversityDetailsPage({ data, universities, onSwitch, programId, onSelectProgram, onSaveStage, onRollbackStage, onUploadStageAttachment, onDeleteStageAttachment, onCreateTask, onUpdateTask, onOpenPrograms, requiresStatusApproval = false, currentUserName }: {
  ...
  onRollbackStage: (programId: number, stageId: number) => Promise<string | void>
  ...
  return <div className="content">
    ...
    <section className="card program-selector"><label htmlFor="program-select">ИТ-направление · программа · ИТ-продукт</label>{selectedProgram ? <><select id="program-select" value={selectedProgram.id} onChange={event => onSelectProgram(Number(event.target.value))}>{programs.map(program => <option key={program.id} value={program.id}>{program.direction || 'Без направления'} · {program.name} · {program.product}</option>)}</select><span>Направление, образовательная программа и используемый ИТ-продукт учитываются отдельно.</span></> : <p>У этого вуза пока нет программ.</p>}</section>

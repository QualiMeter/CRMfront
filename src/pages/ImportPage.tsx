...
export function ImportPage({ universities, onImportUniversities, onImportPrograms }: {
  ...
  function loadSheet(book: NonNullable<typeof workbook>, selectedSheet: string, nextKind = kind) {
    ...
    nextFields.forEach(([key, , , aliases]) => { const match = nextHeaders.find(header => aliases.includes(normalize(header) as never)); if (match) autoMapping[key] = match })
  ...
  return <div className="content import-workspace">
    ...
    {!!headers.length && <><section className="card import-mapping-card"><div className="card-header"><div><h2>Сопоставление столбцов</h2><p>Обязательные поля отмечены звёздочкой</p></div><span className={`import-state ${mappingComplete ? 'ok' : ''}`}>{mappingComplete ? 'Готово' : 'Нужно заполнить'}</span></div><div className="import-mapping-grid">{fields.map(([key, label, required]) => <label key={key}><span>{label}{required && <b> *</b>}</span><select value={mapping[key] ?? ''} onChange={event => setMapping(current => ({ ...current, [key]: event.target.value }))}><option value="">Не импортировать</option>{headers.map(header => <option key={header}>{header}</option>)}</select></label>)}</div></section>
    ...
    <p className="demo-note">Демо: импортированные записи сохраняются до перезагрузки страницы. После подключения backend проверка и запись будут повторяться на сервере.</p>
  </div>
}

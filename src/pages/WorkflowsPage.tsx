...
export function WorkflowsPage({ templates, universities, programs, onCreate, onUpdate, onDelete, onApply }: {
  ...
  return <div className="content workflows-workspace">
    ...
    {message && <p className="workflow-feedback success" role="status">{message}</p>}{error && <p className="workflow-feedback error" role="alert">{error}</p>}<p className="demo-note">Шаблоны, этапы и назначения сохраняются на backend.</p>
  </div>
}
